package com.sony.store.myapplication.widget.tablayout
...
class ScaleTabLayout : HorizontalScrollView {
    ...
    private val title = listOf("推荐产品", "会员专区")
    ...
    private fun initAttr(attrs: AttributeSet?) {
        ...
        mUnSelTextColor =
            ta.getColor(R.styleable.ScaleTabLayout_stl_unSelTextColor, DEF_UN_SEL_TEXT_COLOR)
        mSelTextColor = ta.getColor(R.styleable.ScaleTabLayout_stl_selTextColor, DEF_SEL_TEXT_COLOR)
        mUnSelTextSize = ta.getDimension(
            R.styleable.ScaleTabLayout_stl_unSelTextSize,
            resources.getDimension(R.dimen.stl_un_sel_text_size)
        )
        mSelTextSize = ta.getDimension(
            R.styleable.ScaleTabLayout_stl_selTextSize,
            resources.getDimension(R.dimen.stl_sel_text_size)
        )
        mTabSpacing = ta.getDimension(
            R.styleable.ScaleTabLayout_stl_tabSpacing,
            resources.getDimension(R.dimen.stl_tab_spacing)
        )
        ...
        mRightMargin =
            ta.getDimension(R.styleable.ScaleTabLayout_stl_rightMargin, defLeftRightMargin)
    ...
    private fun initViews() {
        ...
        mLlContainer.layoutParams =
            ViewGroup.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, dip2px(context, 50f))
    ...
    private fun clickTab(position: Int) {
        ...
        if (isClickAnimation) {
            if (isSelTextBold) {
                (mLlContainer.getChildAt(tempOldPagerPosition) as TextView).run {
    ...
    fun setViewPager(viewPager: ViewPager2) {
        ...
        for (index in 0 until viewPager.adapter!!.itemCount) {
            val indexFlag = when (index) {
                0 -> 0
                viewPager.adapter!!.itemCount - 1 -> 1
                else -> -1
            }
    ...
    private fun finalSelect(position: Int) {
        ...
        if (!isClickAnimation) {
            for (index in 0 until mLlContainer.childCount) {
                outScale(index, 0f)
                if (isSelTextBold) {
                    (mLlContainer.getChildAt(index) as TextView).run {
                        typeface = Typeface.defaultFromStyle(Typeface.NORMAL)
                    }
                }
            }
     ...
     */
    private fun generateNewTab(
        index: Int,
        tag: String,
        @IntRange(from = -1, to = 1) indexFlag: Int,
        isSelect: Boolean
        ...
        layoutParams = LayoutParams(
            ...
            when {
                index == 0 || indexFlag == 0 -> {
                    setPadding(mLeftMargin.toInt(), 0, mTabSpacing.toInt(), 0)
                }
                indexFlag == 1 -> setPadding(0, 0, mRightMargin.toInt(), 0)
                else -> setPadding(0, 0, mTabSpacing.toInt(), 0)
            }
        }
    ...
    fun addTab(index: Int, tag: String, isSelect: Boolean = false) {
        if (mLlContainer.childCount > 0) {
            if (index == 0) {
                ...
                oldFirstTabView.layoutParams =
                    (oldFirstTabView.layoutParams as MarginLayoutParams).apply { leftMargin = 0 }
            } else if (mLlContainer.childCount > 0) {
                val lastFirstTabView = mLlContainer.getChildAt(mLlContainer.childCount - 1)
                lastFirstTabView.layoutParams =
                    (lastFirstTabView.layoutParams as MarginLayoutParams).apply {
